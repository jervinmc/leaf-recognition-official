...
def main():
    ...
    imgBlurred = cv2.GaussianBlur(imgGray, (35,35), 0)                        # blur

                                                        # filter image from grayscale to black and white
    _,imgThresh = cv2.threshold(imgBlurred, 127, 255,
                               cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)                                    # constant subtracted from the mean or weighted mean
    ...
    for npaContour in npaContours:                          # for each contour
        if cv2.contourArea(npaContour) > MIN_CONTOUR_AREA:          # if contour is big enough to consider
            ...
            else:      # else if the char is in the list of chars we are looking for . . .
    ...
    np.savetxt("classification.txt", npaClassifications)           # write flattened images to file
    np.savetxt("features.txt", npaFlattenedImages)          #
